...
def create_category(category: CategoryCreate, db: Session = Depends(get_db), admin_user: UserModel = Depends(require_admin)):
...
def update_category(category_id: int, category_update: CategoryUpdate, db: Session = Depends(get_db), admin_user: UserModel = Depends(require_admin)):
...
def delete_category(category_id: int, db: Session = Depends(get_db), admin_user: UserModel = Depends(require_admin)):
...
def create_menu_item(item: MenuItemCreate, db: Session = Depends(get_db), admin_user: UserModel = Depends(require_admin)):
...
def update_menu_item(item_id: int, item_update: MenuItemUpdate, db: Session = Depends(get_db), admin_user: UserModel = Depends(require_admin)):
    ...
    if item_update.category_ids is not None:
        categories = db.query(CategoryModel).filter(CategoryModel.id.in_(item_update.category_ids)).all()
        db_item.categories = categories
...
def delete_menu_item(item_id: int, db: Session = Depends(get_db), admin_user: UserModel = Depends(require_admin)):
